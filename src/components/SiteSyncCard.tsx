...
type Props={result:{id:string;siteId?:string;siteSync?:SiteSync};sites:{id:string;name:string}[];call:<T>(path:string,body?:unknown)=>Promise<T>};
export default function SiteSyncCard({result,sites,call}:Props){
 ...
 return <div className="gam-site-sync">
  {saved?<><strong>✓ Upisano u sajt: {saved.siteName||saved.siteId}</strong><p>{saved.addedUnits} novih ad unita · {saved.addedMaps} novih mapa. GAM ID-jevi i putanje su sačuvani. Postojeća podešavanja su zadržana.</p><small>Generate / Publish primenjuje novi inventar na sajtu.</small></>:<>
   ...
   {review&&<div className="gam-review-summary"><strong>{review.siteName}</strong><span>{review.addedUnits} novih ad unita · {review.existingUnits} postojećih · {review.addedMaps.length} novih mapa</span>{review.warnings.map((w,i)=><small key={i}>{w}</small>)}<div className="gam-table-wrap"><table><thead><tr><th>Ad unit</th><th>Size mapa</th><th>Sajt</th></tr></thead><tbody>{review.rows.map(r=><tr key={r.code}><td>{r.code}</td><td>{r.mapKey}</td><td>{r.state==='new'?'Dodaje se':'Zadržava se'}</td></tr>)}</tbody></table></div><button className="gam-primary" type="button" disabled={busy} onClick={()=>void run(async()=>{setSaved(await call<SiteSync>('/site-sync/apply',{id:review.id,confirmSite:review.siteId}));setReview(null);})}>Potvrdi upis u sajt</button></div>}
  </>}
