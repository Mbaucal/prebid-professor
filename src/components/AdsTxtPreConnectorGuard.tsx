...
export default function AdsTxtPreConnectorGuard() {
  useEffect(() => {
    ...
    const apply = () => {
      ...
      // The managed source editor is intentionally unavailable until this
      // site has an authenticated CMS/API publishing connector.
      document.getElementById('ads-txt-source-editor-portal')?.remove();

      // Before a connector exists, the Ads.txt workspace remains a checker
      // against canonical platform requirements. Do not mix read-only live
      // rows into the editable requirement-search results.
