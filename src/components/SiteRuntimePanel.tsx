...
export default function SiteRuntimePanel({publisherId,view,unitCode,endpoint,onChanged}:Props){
 ...
 return <section className="site-runtime-panel" aria-label={view==='versions'?'Script versions':'Ad position display settings'}>
  ...
  {state&&view==='versions'?<>
   ...
   <label>Script version<select value={version} disabled={busy} onChange={e=>{setVersion(e.target.value);setApproved(false);}}><option value="">Choose a version</option>{state.runtimes.map(r=><option key={r.pin.runtimeSha256} value={r.pin.runtimeSha256}>{r.version}</option>)}</select></label>
