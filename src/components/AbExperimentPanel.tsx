...
type State = {supported:boolean; revision:string; defaults:Settings; baseline:{release:string;positions:number;prebidVersion:string;sha256:string;bytes:number}; packages:Saved[]; nextCursor:string|null};
...
export default function AbExperimentPanel({publisherId}:{publisherId:string}) {
  ...
  async function load(more=false) {
    ...
    setState(old=>more&&old?{...data,packages:[...new Map([...old.packages,...data.packages].map(p=>[p.release,p])).values()].sort((a,b)=>b.createdAt.localeCompare(a.createdAt))}:data);
  }
  ...
  return <section className="ab-editor" aria-label="A/B testing">
    ...
    {!state?<p>Loading A/B packages…</p>:<>
      ...
      <form onSubmit={generate}>
        <fieldset disabled={busy} className="ab-form">
          ...
          <div className="ab-arms">{(['A','B'] as const).map(v=><fieldset key={v} className="ab-arm"><legend>Variant {v}</legend>
            <label>Auction mode<select value={arms[v].mode} onChange={e=>update(v,{mode:e.target.value as Arm['mode']})}><option value="fresh-only">Fresh auction</option><option value="auction-with-cache">Auction + valid cached bids</option></select></label>
            ...
            <label>Refresh<select value={arms[v].refreshMode} onChange={e=>update(v,{refreshMode:e.target.value as ArmForm['refreshMode']})}><option value="preserve">Keep baseline position rules</option><option value="fixed">Set standard interval</option></select></label>
      ...
      {!state.packages.length?<p>No A/B packages generated yet.</p>:state.packages.map(p=><article className="ab-saved" key={p.release}>
        ...
        <div className="runtime-actions"><button className="button primary" disabled={busy} onClick={()=>void run(()=>download(p.release,p.release,p))}>Download A/B ZIP</button><button className="button secondary" disabled={busy} onClick={()=>useSettings(p)}>Use these settings</button></div>
