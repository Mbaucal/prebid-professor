...
import { testPageUrl } from './SiteTestPagePanel';
...
type Props={publisherId:string;endpoint?:string;settingsEndpoint?:string;onChanged?:()=>void|Promise<void>;onNavigate?:(destination:'prebid'|'versions'|'demand')=>void};
export default function SitePackagesPanel({publisherId,endpoint,settingsEndpoint,onChanged,onNavigate}:Props){
 ...
 {(!state.ready && state.nextStep) || editingSetup ? <ScriptSetupPanel
   publisherId={publisherId} endpoint={settingsEndpoint}
   onOpenPrebid={onNavigate?()=>onNavigate('prebid'):undefined}
   onChanged={async()=>{await reload();setEditingSetup(false);await onChanged?.();}}
 /> : <>
   {state.error?<p role="alert" className="runtime-error">{state.error}</p>:null}
   <div className="script-version-summary"><span><strong>{state.enablePrebid?'GAM + Prebid':'GAM / AdX only'}</strong> · {state.runtime?runtimeLabel(state.runtime.runtimeVersion):'Setup needed'}</span><button disabled={busy} onClick={()=>setEditingSetup(true)}>Change script setup</button></div>
 </>}
 {editingSetup && state.ready ? <button disabled={busy} onClick={()=>setEditingSetup(false)}>Cancel setup changes</button> : null}
 <h3>2. Generate your script</h3>
 <p>Create a ZIP from your saved ad units and size maps, then download it below.</p>
 ...
 <div className="runtime-actions"><button disabled={busy||!state.ready||editingSetup} onClick={()=>void run(generate)}>{busy?'Working…':'Generate and save package'}</button><button disabled={busy} onClick={()=>void run(reload)}>Reload releases</button></div>
 ...
 <h3>Saved versions</h3>{state.releases.length===0?<p>No built-in packages saved yet.</p>:state.releases.map((r:any)=><article key={r.id} className="runtime-release"><strong>{r.notes||'Generated package'}</strong><p>{new Date(r.createdAt).toLocaleString()} · {r.status}</p><details><summary>Package ID</summary><code>{r.id}</code></details><div className="runtime-actions"><button disabled={busy} onClick={()=>void run(()=>download(r.id))}>Download saved ZIP</button><a className="button" href={testPageUrl(publisherId,r.id,state.testOnly)} target="_blank" rel="noopener noreferrer">Open test page</a>{!state.testOnly?<><button disabled={busy||r.status==='production'} onClick={()=>void run(()=>channel(r.id,'staging'))}>Stage package</button>{r.status==='staging'?<button disabled={busy} onClick={()=>void run(()=>channel(r.id,'production'))}>Publish package</button>:null}{r.status==='archived'?<button disabled={busy} onClick={()=>void run(()=>channel(r.id,'rollback'))}>Restore published package</button>:null}{deleteButton(r)}</>:null}</div></article>)}
