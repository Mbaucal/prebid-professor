...
export default function SitePackagesPanel({publisherId,endpoint,onChanged}:Props){
 ...
 async function download(id:string){const r=await request({action:'download',releaseId:id});const object=URL.createObjectURL(await r.blob()),a=document.createElement('a');a.href=object;a.download=`${id}.zip`;a.click();setTimeout(()=>URL.revokeObjectURL(object),1000);setMessage('Saved package downloaded.');}
