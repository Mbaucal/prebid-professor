...
export default function SiteTestPagePanel({ publisherId, endpoint, testOnly=false }:Props) {
  ...
  return <section className="site-runtime-panel" aria-label="Test page">
    ...
    {loading ? <p>Loading saved packages…</p> : releases.length ? <>
      <label>Saved package<select value={selected} onChange={event => setSelected(event.target.value)}>{releases.map(release => <option key={release.id} value={release.id}>{release.notes || 'Generated package'} · {new Date(release.createdAt).toLocaleString()} · {release.status} · {release.id.slice(-8)}</option>)}</select></label>
