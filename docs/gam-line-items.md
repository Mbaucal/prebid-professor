...
- Native workerd using both the compiled TEST and production Workers, real local R2, synthetic signed OAuth and SOAP responses. Outgoing XML types and field order are checked against the v202608 contract. Both flows and replay are exercised.
...
These checks use synthetic Google data. The owner accepted the ordinary line-item flow in live GAM, approved the editable original-script Prebid preset and explicitly requested main promotion on 2026-09-22 ("Spoji u main!"). TEST PRs #76, #79 and #82 are the reviewed source. A full live Prebid run is not claimed by these checks. Production uses the existing authenticated GAM route and its saved encrypted connections; no TEST credentials, jobs or history are migrated.
