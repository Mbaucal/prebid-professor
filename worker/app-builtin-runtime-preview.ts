...
export default {
  async fetch(request, env, ctx): Promise<Response> {
    ...
    const match = url.pathname.match(/^\/api\/publishers\/([^/]+)\/(builtin-runtime-preview|builtin-runtime-prebid-check)$/);
    ...
    if (prebidCheck ? request.method !== 'GET' : !['GET', 'POST'].includes(request.method)) return fail('Method not allowed.', 405);
