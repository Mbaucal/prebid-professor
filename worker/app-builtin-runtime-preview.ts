...
import { packageResponse, builtInCdn } from './site-runtime/releases.mjs';
...
export default {
  async fetch(request, env, ctx): Promise<Response> {
    ...
    const packageMatch=url.pathname.match(/^\/api\/publishers\/([a-z0-9][a-z0-9-]{0,97})\/builtin-releases$/);
    if(packageMatch){
