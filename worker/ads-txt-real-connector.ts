...
async function ensureTables(db: D1Database): Promise<void> {
  if (!tablesReady) {
    tablesReady = db.prepare(`CREATE TABLE IF NOT EXISTS ads_txt_real_connectors (
...
function normalizedAuthHeader(authType: string, value: unknown): string {
  ...
  if (!/^[A-Za-z0-9-]+$/.test(header)) throw new Error('Authorization header name is not valid.');
  return header;
}
...
export async function saveAdsTxtRealConnector(
  ...
  try {
    ...
    return apiError('Connection settings are not valid JSON.', 400, error instanceof Error ? error.message : String(error));
  }
  ...
  let credentialEncrypted = authType === 'none' ? null : existing?.credential_encrypted ?? null;
