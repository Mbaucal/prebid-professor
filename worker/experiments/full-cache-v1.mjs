...
import {createCacheLifecycle} from '../runtime-cache/lifecycle.mjs';
import {createConsentEpoch} from '../runtime-cache/consent-epoch.mjs';

export const CACHE_RELEASE='tanjug-cache-1.0.0';
export function cacheArm(source,variant){
  ...
  output=output.replace(marker,marker+`
    ...
    function ensureCacheLifecycle(){
      if(!cacheLifecycle)cacheLifecycle=(${createCacheLifecycle.toString()})({win:window,pbjs:pbjs,
