import { createAssignmentSender } from './assignment-sender.mjs';
...
// New opt-in TEST loader version. Original delivery.mjs and packaged runtimes stay unchanged.
export const COLLECTED_PROFILE = 'experiment-collected-preview-v1';
...
function validate(input) {
  ...
  requireThat(input.profile === COLLECTED_PROFILE && typeof input.siteId === 'string' && NAME.test(input.siteId)
...
function boot(context, assets, receipt, createSender) {
  ...
  var sender = createSender(current, receipt);
  ...
  var state = { context: Object.freeze(context), status: 'loading',
    ...
    snapshot: function () { return { context: state.context, events: events.slice(), loaderAttempts: attempts, blockedDuplicates: attempts - 1, collection: sender.snapshot() }; } };
 ...
 */
export async function createCollectedDelivery(input, packages, { random = Math.random, issueTicket } = {}) {
  ...
  const loaderSha256 = await sha256(new TextEncoder().encode(boot.toString() + createAssignmentSender.toString()));
  ...
  return Object.freeze({
    ...
    // Ticket issuance is supplied by the authenticated TEST adapter.
    async fetch(request) {
      ...
      if (url.pathname === '/ads.js') {
        ...
        const context = {runtimeVersion:selected.runtimeVersion, prebidVersion:selected.prebidVersion, profile:'experiment-preview-v1', deliveryProfile:config.profile, siteId:config.siteId, experimentId:config.experimentId,
        ...
        if(config.enabled && request.method === 'GET') {
          try { receipt = {ticket:await issueTicket(context)}; } catch { receipt = {unavailable:true}; }
        }
        const body = '(' + boot.toString() + ')(' + encode(context) + ',' + encode({ads:integrity(scriptHashes.get(pin)),prebid:prebidHashes.has(pin)?integrity(prebidHashes.get(pin)):null}) + ',' + encode(receipt) + ',' + createAssignmentSender.toString() + ');\n';
