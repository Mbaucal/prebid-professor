import { runtimeDescriptor, generatePreview } from './builtin-preview-service.mjs';
import { previewInput, digest } from './preview-snapshot.mjs';
import { assertPinnedRuntime } from './version-pin.mjs';
import { prebidRequirements, sha256 } from './prebid-artifact-check.mjs';
import { finalizeJavaScript } from './artifact-minifier.mjs';
import { stickyStyles, installSharedStickyStyles, placeholderStyles } from './artifact-styles.mjs';
...
export async function buildArtifactCandidate({ snapshot, takeOver = { enabled: false }, pin, buildTimestamp, prebid = null }) {
  ...
  const generated = generatePreview(snapshot, takeOver, pin, buildTimestamp);
  ...
  files['config.json'] = text.encode(`${JSON.stringify({ schemaVersion: 1, siteId: snapshot.site.id, runtime: pin,
    core: input.core, options: input.options, output: { cleanComments }, prebidBuild: prebidPin }, null, 2)}\n`);
  const units = input.core.explicitUnits.filter((u) => u.id !== input.options.sticky.bottomAdUnitId);
