...
import {descriptor as creative} from '../../.generated/runtime-creative-manifest.mjs';
import {buildArtifactCandidate as creativeCandidate} from '../runtime-creative-v1/artifact-candidate.mjs';
...
export const runtimeCatalog=[validateRuntimeDescriptor(next),reference,validateRuntimeDescriptor(creative),validateRuntimeDescriptor(reporting)];
...
export function previewInput(snapshot,descriptor,time,takeOver){
  ...
  if((descriptor.id===next.id||descriptor.id===creative.id||descriptor.id===reporting.id)){
...
export async function buildArtifactCandidate(args){
  ...
  return descriptor.id===reporting.id?reportingCandidate(args):descriptor.id===creative.id?creativeCandidate(args):descriptor.id===next.id?positionsCandidate(args):referenceCandidate(args);
}
