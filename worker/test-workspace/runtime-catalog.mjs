...
export const runtimeCatalog=[validateRuntimeDescriptor(next),reference,validateRuntimeDescriptor(reporting)];
...
export function previewInput(snapshot,descriptor,time,takeOver){
  ...
  if(descriptor.id===next.id||descriptor.id===reporting.id){
...
export async function buildArtifactCandidate(args){
  ...
  return descriptor.id===reporting.id?reportingCandidate(args):descriptor.id===next.id?positionsCandidate(args):referenceCandidate(args);
}
