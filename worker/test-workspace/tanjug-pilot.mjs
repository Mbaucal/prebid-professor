...
<div class="actions"><a class="button primary" href="/test-api/pilots/tanjug/download">Preuzmi Tanjug ZIP</a><a class="button" href="/deployments">TEST objave i verzije</a><a class="button" href="/test-api/pilots/tanjug/prebid-config">Preuzmi prebid-config.json</a></div>
...
const ui = `const frame=document.getElementById('pilot-preview');for(const mode of ['desktop','mobile'])document.getElementById(mode).addEventListener('click',()=>{frame.style.width=mode==='mobile'?'392px':'100%';for(const id of ['desktop','mobile'])document.getElementById(id).setAttribute('aria-pressed',String(id===mode));frame.src='/pilot/tanjug/preview';});new ResizeObserver(()=>{document.getElementById('preview-size').textContent='Širina prikaza: '+frame.clientWidth+' px';}).observe(frame);`;
