...
export const runtimeCatalog=[validateRuntimeDescriptor(next),reference,observed,measured];
...
export function previewInput(snapshot,descriptor,time,takeOver){
  ...
  if(descriptor.id===next.id||descriptor.id===observed.id||descriptor.id===measured.id){
...
export async function buildArtifactCandidate(args){
  ...
  return descriptor.id===measured.id?measuredCandidate(args):descriptor.id===observed.id?observedCandidate(args):descriptor.id===next.id?positionsCandidate(args):referenceCandidate(args);
}
