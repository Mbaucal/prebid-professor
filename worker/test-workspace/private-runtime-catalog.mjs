...
export const runtimeCatalog=[validateRuntimeDescriptor(next),reference,observed];
...
export function previewInput(snapshot,descriptor,time,takeOver){
  ...
  if(descriptor.id===next.id||descriptor.id===observed.id){
...
export async function buildArtifactCandidate(args){
  ...
  return descriptor.id===observed.id?observedCandidate(args):descriptor.id===next.id?positionsCandidate(args):referenceCandidate(args);
}
