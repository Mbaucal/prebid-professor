...
const OAUTH_SCOPE = [...GOOGLE_IDENTITY_SCOPES, GMAIL_SEND_SCOPE].join(' ');
...
export async function getGmailStatus(env: GmailOAuthEnv): Promise<Response> {
  ...
  const row = await env.DB.prepare(
    `SELECT email, scope, connected_by, connected_at, updated_at
     FROM gmail_connections
     WHERE id = 'primary'
     LIMIT 1`,
  ).first<Omit<GmailConnectionRow, 'refresh_token_encrypted'>>();
  return json({
    ok: true,
...
export async function startGmailConnect(request: Request, env: GmailOAuthEnv): Promise<Response> {
  ...
  await env.DB.prepare(
    `INSERT INTO gmail_oauth_states (state, actor, redirect_uri, expires_at, created_at)
     VALUES (?, ?, ?, ?, ?)`,
  )
    .bind(
      state,
...
export async function finishGmailConnect(request: Request, env: GmailOAuthEnv): Promise<Response> {
  ...
  const stateRow = await env.DB.prepare(
    `SELECT actor, redirect_uri, expires_at
     FROM gmail_oauth_states
     WHERE state = ?
     LIMIT 1`,
  )
    .bind(state)
    .first<{ actor: string; redirect_uri: string; expires_at: string }>();

  if (!stateRow || new Date(stateRow.expires_at).getTime() < Date.now()) {
    return apiError('Gmail OAuth state is invalid or expired.', 400);
  }
  ...
  if (!tokenResponse.ok || !tokenPayload.access_token || !tokenPayload.refresh_token) {
    return apiError(
      ...
      502,
      tokenPayload.error_description || tokenPayload.error || tokenPayload,
    );
  }
  ...
  if (!identityResponse.ok || !identity.email || identity.email_verified === false) {
    return apiError(
      'Connected Google account could not be identified.',
      502,
  ...
  await env.DB.prepare(
    `INSERT INTO gmail_connections (
       id, email, refresh_token_encrypted, scope, connected_by, connected_at, updated_at
     ) VALUES ('primary', ?, ?, ?, ?, ?, ?)
    ...
    .bind(
      ...
      encrypted,
      tokenPayload.scope || OAUTH_SCOPE,
      stateRow.actor,
      now,
      now,
    )
...
export async function getStoredGmailConnection(env: GmailOAuthEnv): Promise<GmailConnectionRow | null> {
  ...
  return env.DB.prepare(
    `SELECT email, refresh_token_encrypted, scope, connected_by, connected_at, updated_at
     FROM gmail_connections
     WHERE id = 'primary'
     LIMIT 1`,
