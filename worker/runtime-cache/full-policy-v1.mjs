// New full-layout profile. Previously shipped policy bytes remain unchanged.
...
export function createFullBidCachePolicy({pbjs,siteId,contextForCode,cacheAllowed=()=>true,auctionAllowed=()=>true,mode='fresh-only',maxAgeSeconds=30,now=Date.now}) {
  ...
  return {
    ...
    snapshot(){return {profile:'full-banner-cache-policy-v1',siteId,prebidVersion:pbjs.version,mode,maxAgeSeconds,stopped,
