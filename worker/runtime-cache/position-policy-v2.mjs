...
export function createPositionBidCachePolicy({pbjs,siteId,contextForCode,cacheAllowed=()=>true,auctionAllowed=()=>true,mode='fresh-only',maxAgeSeconds=30,defaultCacheEnabled=mode==='auction-with-cache',positionOverrides={},now=Date.now}) {
  ...
  if(typeof defaultCacheEnabled!=='boolean'||!positionOverrides||typeof positionOverrides!=='object'||Array.isArray(positionOverrides)
    ||Object.keys(positionOverrides).length>100||Object.entries(positionOverrides).some(([code,value])=>!code||typeof value!=='boolean'))throw Error('Invalid position cache settings.');
  const positions=Object.freeze({...positionOverrides});
  const cacheEnabledFor=code=>Object.hasOwn(positions,code)?positions[code]:defaultCacheEnabled;
  const useCache=mode==='auction-with-cache'&&(defaultCacheEnabled||Object.values(positions).some(Boolean));
  ...
  function context(code) {
    try{
      ...
      if(c.sizes.some(s=>s!=='fluid'&&(!Array.isArray(s)||s.length!==2||!s.every(v=>Number.isInteger(v)&&v>=1&&v<=10000))))return null;
      return {epoch:c.epoch,slot:c.slot,sizes:c.sizes.map(s=>s==='fluid'?'fluid':s.join('x')).sort(),targeted:false};
    }catch{return null;}
  }
  ...
  function reason(bid) {
    if(stopped||!useCache)return 'disabled';
    if(!cacheEnabledFor(bid?.adUnitCode))return 'position-disabled';
    try{if(cacheAllowed()!==true)return 'consent';}catch{return 'consent';}
    if(capacityBlocked)return 'capacity';
    if(!bid||bid.mediaType!=='banner'||!Number.isInteger(bid.width)||!Number.isInteger(bid.height)||bid.width<=1||bid.height<=1)return 'format';
  ...
  try{pbjs.setConfig({useBidCache:useCache,bidCacheFilterFunction:filter,customGptSlotMatching:match});}
  ...
  return {
    ...
    target(code,auctionId) {
      ...
      try{
        ...
        if(pbjs.getConfig('customGptSlotMatching')!==match||pbjs.getConfig('bidCacheFilterFunction')!==filter||pbjs.getConfig('useBidCache')!==useCache)throw Error('Policy configuration changed.');
        ...
        for(const id of ids){
          ...
          if(bid.auctionId!==latest.get(code)&&(!useCache||!cacheEnabledFor(code)))throw Error('Cached targeting is disabled for this position.');
    ...
    snapshot(){return {profile:'position-banner-cache-policy-v2',defaultCacheEnabled,positionOverrides:{...positions},siteId,prebidVersion:pbjs.version,mode,maxAgeSeconds,stopped,
