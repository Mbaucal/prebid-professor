...
export async function prepareTanjugCacheReview({proposal,sourceBytes,prebidBytes}){
 ...
 const result={version:proposal.version,status:'offline-review-only',source:{file:proposal.sourceFile,sha256:proposal.sourceSha256,version:source.version,liveConfigurationVerified:false},runtime:pin,prebidSha256:PREBID_SHA256,
  ...
  blockersBeforeHostedPilot:['Confirm the isolated page hostname and its actual Google Funding Choices installation.','Refresh the Tanjug baseline: this proposal uses the frozen 14 September TEST source, not verified current live settings.','Review the two-position scope and 60-second age cap before activating a pilot.','Verify real CMP/GPT, actual BFCache return, long sessions and A/A reporting before any revenue comparison.'],
