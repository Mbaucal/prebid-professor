import { descriptor as rawDescriptor } from '../../.generated/runtime-demand-manifest.mjs';
import { validateRuntimeDescriptor } from '../runtime/version-pin.mjs';
import { compileDemand } from './compiler.mjs';
import { demandReportingContract } from './contract.mjs';
const runtimeDescriptor=validateRuntimeDescriptor(rawDescriptor);
import { previewInput } from './snapshot.mjs';
...
import { prebidRequirements } from './requirements.mjs';
...
export async function buildArtifactCandidate({ snapshot, takeOver = { enabled: false }, pin, buildTimestamp, prebid = null }) {
  ...
  if (requirements.required) {
    ...
    if (!prebid.report.declaredModules.includes('gptPreAuction')) throw new Error('Build Prebid with gptPreAuction before selecting this script.');
  ...
  const generated = compileDemand(input);
  ...
  files['gam-reporting.json'] = text.encode(JSON.stringify(demandReportingContract(input), null, 2) + '\n');
  ...
  files['config.json'] = text.encode(`${JSON.stringify({ schemaVersion: 1, siteId: snapshot.site.id, runtime: pin,
    core: input.core, options: input.options, demandSignals: input.demandSignals, adPosition: input.overlay, lazyRules: input.lazyRules, output: { cleanComments }, prebidBuild: prebidPin }, null, 2)}\n`);
...
${htmlUnits}
  ...
  files['README.txt'] = text.encode(new TextDecoder().decode(files['README.txt']) +
    '\nGAM reporting\nThis version adds seven slot-level request labels. GAM-only sends refresh_count, refresh_bucket, refresh_interval and refresh_policy, with no aq_* values. Completed Prebid auctions also report positive current-auction bid counts and the auction timeout flag. See gam-reporting.json for every value and its meaning.\n\nCreate the custom targeting keys/values and enable reporting in GAM before collecting results. Downloading or selecting this version does not configure GAM reporting. Counts are requests and bids, not impressions, winning demand or revenue. Old packages retain their original behavior.\n');
  if (requirements.required) files['README.txt'] = text.encode(new TextDecoder().decode(files['README.txt']) +
    '\nPrebid targeting and GPID\nSend All Bids and alwaysIncludeDeals are enabled. Standard winner keys remain alongside bidder-specific keys. gam-reporting.json lists the expected keys for the configured bidders; do not enable high-cardinality hb_adid values as reporting dimensions. Configure the corresponding GAM line items and reporting separately.\n\nEvery position has a stable GPID based on its full GAM path and saved position code, or an explicit publisher override. Refresh, viewport and A/B assignment do not change that identity. gptPreAuction is required in the selected Prebid build. Verify GPID in actual outbound partner requests before production use.\n');
