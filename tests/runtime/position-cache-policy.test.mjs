...
import { createPositionBidCachePolicy } from '../../worker/runtime-cache/position-policy-v2.mjs';
function fixture(options={}){
  ...
  const policy=createPositionBidCachePolicy({pbjs,siteId:'test-site',contextForCode:code=>code==='P1'?{slot,epoch,sizes}:null,mode:'auction-with-cache',now:()=>time,...options});
...
test('bounded tracking, explicit fresh-only mode and teardown cannot silently turn caching on',()=>{
  ...
  assert.throws(()=>createPositionBidCachePolicy({pbjs:{version:'11.33.0'}}),/11.34.0/);
...
test('mixed layout retains fresh 1x1 but never reuses it, and keeps numeric banner caching',()=>{
 const f=fixture();f.sizes(['fluid',[1,1],[300,250]]);f.auction('old');f.auction('new');
 assert.equal(f.config.bidCacheFilterFunction(f.bid),true);
 assert.equal(f.config.bidCacheFilterFunction({...f.bid,width:1,height:1}),false);
 f.bid.auctionId='new';f.bid.width=1;f.bid.height=1;
 assert.equal(f.policy.target('P1','new'),true);assert.equal(f.policy.snapshot().selections.fresh,1);
});

test('per-position off excludes old bids but accepts a fresh bid; explicit on overrides the site default',()=>{
  const off=fixture({positionOverrides:{P1:false}});
  assert.equal(off.config.useBidCache,true);
  assert.equal(off.config.bidCacheFilterFunction(off.bid),false);
  assert.equal(off.policy.snapshot().checks.rejected['position-disabled'],1);
  // Simulate a broken native filter: the post-targeting guard must clear the stale result.
  assert.equal(off.policy.target('P1'),false);assert.equal(off.values.hb_adid,undefined);
  off.auction('third-refresh');off.bid.auctionId='third-refresh';off.bid.status='good';
  assert.equal(off.policy.target('P1','third-refresh'),true);
  assert.equal(off.policy.snapshot().lastSelection.origin,'fresh');
  const on=fixture({defaultCacheEnabled:false,positionOverrides:{P1:true}});
  assert.equal(on.config.useBidCache,true);assert.equal(on.config.bidCacheFilterFunction(on.bid),true);
  assert.equal(on.policy.target('P1'),true);assert.equal(on.policy.snapshot().lastSelection.origin,'cache');
});
test('all off disables native reuse and snapshots cannot alter position choices',()=>{
  const off=fixture({defaultCacheEnabled:false,positionOverrides:{P1:false}});
  assert.equal(off.config.useBidCache,false);assert.equal(off.config.bidCacheFilterFunction(off.bid),false);
  off.bid.auctionId='new';assert.equal(off.policy.target('P1'),true);
  const settings={P1:false},on=fixture({positionOverrides:settings});settings.P1=true;
  const snapshot=on.policy.snapshot();snapshot.positionOverrides.P1=true;
