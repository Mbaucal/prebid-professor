...
import { createFullBidCachePolicy } from '../../worker/runtime-cache/full-policy-v1.mjs';
function fixture(options={}){
  ...
  const policy=createFullBidCachePolicy({pbjs,siteId:'test-site',contextForCode:code=>code==='P1'?{slot,epoch,sizes}:null,mode:'auction-with-cache',now:()=>time,...options});
...
test('bounded tracking, explicit fresh-only mode and teardown cannot silently turn caching on',()=>{
  ...
  assert.throws(()=>createFullBidCachePolicy({pbjs:{version:'11.33.0'}}),/11.34.0/);
