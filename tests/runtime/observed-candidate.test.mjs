...
import { buildArtifactCandidate,runtimeDescriptor } from '../../worker/runtime-observed/artifact-candidate.mjs';
...
test('new observed package is independently pinned and leaves the old generator byte-identical',async()=>{
  ...
  assert.equal(checked.descriptor.runtime.runtimeVersion,'3.11.0-tessera.preview.1');
  assert.match(new TextDecoder().decode(candidate.files['ads.js']),/__tesseraRuntimeDiagnostics/);
  ...
  await mkdir('.generated/observed-evidence',{recursive:true});
  await writeFile('.generated/observed-evidence/ads.js',candidate.files['ads.js']);
});
