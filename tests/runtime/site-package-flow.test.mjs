...
test('main demand-mode API rejects TakeOver conflicts, synchronizes the exact Prebid pin and preserves concurrent settings',async()=>{
 ...
 assert.deepEqual(JSON.parse(transitions[1].details_json),{previousEnabled:true,enabled:false,mode:'gam-adx-only',savedBidderConfigurationPreserved:true,bidCache:{enabled:false,maxBidAgeSeconds:60}});
 assert.deepEqual(JSON.parse(transitions[0].details_json),{previousEnabled:false,enabled:true,mode:'gam-prebid',savedBidderConfigurationPreserved:true,bidCache:{enabled:false,maxBidAgeSeconds:60}});
