...
__testAds.service.addEventListener('slotRequested',({slot})=>{
  ...
  fixtureRequests.push({id:slot.id,cpm:bid?.cpm??null,status:bid?.status??null,ageMs:bid?Date.now()-bid.responseTimestamp:null,
    version:slot.getTargeting('hb_ver'),englishVariant:slot.getTargeting('Variant'),experiment:slot.getTargeting('Variant').length?slot.getTargeting('Variant'):slot.getTargeting('Varijant').length?slot.getTargeting('Varijant'):slot.getTargeting('tessera_ab'),publicVariant:slot.getTargeting('Varijant'),legacyExperiment:slot.getTargeting('tessera_ab')});
});
