...
import {createFullCacheLifecycle} from '../../worker/runtime-cache/full-lifecycle-v1.mjs';
...
function lifecycle({blockedByCmp=false}={}){
  ...
  const pbjs={version:'11.34.0',setConfig:c=>Object.assign(config,c),getConfig:k=>k.split('.').reduce((v,p)=>v?.[p],config),
    ...
    requestBids(input){nativeCalls.push(input);if(!blockedByCmp)emit('auctionInit',{auctionId:input.auctionId,adUnitCodes:['P1']});},
  ...
  const api=createFullCacheLifecycle({win:f.win,pbjs,siteId:'test',runtimeVersion:'3.13.0',mode:'auction-with-cache',maxAgeSeconds:30,
...
test('CMP abort callback without auctionInit preserves one clean GAM fallback and no cache targeting',()=>{
 const f=lifecycle({blockedByCmp:true});f.request();
 f.nativeCalls[0].bidsBackHandler({},false,f.nativeCalls[0].auctionId);
 assert.equal(f.requests.length,1);assert.equal(f.targetCalls(),0);
 assert.equal(f.api.snapshot().totals.fallbacks,1);
 assert.equal(f.api.snapshot().policy.trackedAuctions,0);
 assert.equal(f.requests[0][0].hb_adid,undefined);
 f.lateInit();f.nativeCalls[0].bidsBackHandler();f.api.refresh([f.slot]);
 assert.equal(f.requests.length,1);assert.equal(f.targetCalls(),0);f.api.stop();
});
