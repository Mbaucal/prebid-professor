...
test('existing disabled User ID records and private settings are retained through a bidder-only edit',async()=>{
  const f=await ready(),c=JSON.parse((await snap(f)).config.config_json),custom={name:'customId',enabled:false,moduleCode:'customIdSystem',params:{private:'retained'},notes:'Keep'};
  c.userIdConfig={enabled:false,modules:[custom,{name:'id5Id',moduleCode:'id5IdSystem',enabled:false,params:{partner:42},storage:null,notes:'Partner note'}]};
  f.sqlite.prepare('UPDATE publisher_configs SET config_json=?').run(JSON.stringify(c));
  const r=await request(f);assert.equal(r.options.userIds.find(id=>id.name==='id5Id').settings.params.partner,42);
  const p=await plan(f,r);r.draft.buildId=await upload(f,p.configuration);await activate(f,r);
  const saved=JSON.parse((await snap(f)).config.config_json).userIdConfig;
  assert.deepEqual(saved.modules.find(id=>id.name==='customId'),custom);assert.equal(saved.modules.find(id=>id.name==='id5Id').notes,'Partner note');assert.equal(saved.enabled,false);
});
