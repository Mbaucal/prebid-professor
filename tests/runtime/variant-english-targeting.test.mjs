...
import {createMeasurement} from '../../worker/runtime-variant-en/targeting.mjs';
...
const release=runtimeReleaseHistory.find(r=>r.version==='3.14.1');
...
function measurement(patch={},status='loading') {
 const context={profile:'experiment-preview-v1',siteId:'test-site',runtimeVersion:'3.14.1',active:true,variant:'A',experimentId:item.id,revision:1,deliverySha256:'a'.repeat(64),packageSha256:pin.packageSha256,...patch};
 const window={__tesseraExperimentOwner:'test-site',__tesseraExperiments:{'test-site':{context,status}}};
 const apply=vm.runInNewContext('('+createMeasurement.toString()+')("test-site","3.14.1")',{window});
...
test('public key is exactly Variant=A/B, with no delivery ID or branded key in targeting',()=>{
 for(const variant of ['A','B']) {
  const f=measurement({variant});f.apply(f.slot);f.context.variant=variant==='A'?'B':'A';f.apply(f.slot);
  assert.deepEqual(f.slot.targeting,{hb_bidder:'preserved',publisher:'preserved',Variant:variant});
  assert.equal(f.snapshot().key,'Variant');assert.equal(f.snapshot().value,variant);assert.equal(f.snapshot().appliedSlots,1);
 ...
 for(const patch of [{active:false},{variant:'C'},{runtimeVersion:'3.13.0'},{siteId:'other'},{deliverySha256:'bad'}]) {
  const f=measurement(patch);f.apply(f.slot);assert.equal(f.slot.targeting.Variant,undefined);
 }
 for(const status of ['conflict','load-error']){const f=measurement({},status);f.apply(f.slot);assert.equal(f.slot.targeting.Variant,undefined);}
});
test('simple values can be reused while exact private mappings and legacy collision checks remain',async()=>{
 ...
 assert.equal(p.key,'Variant');assert.deepEqual(p.arms.map(a=>a.value),['A','B']);assert(p.labelsReady);
 assert(reportingCsv(p).includes('"Variant","A"'));assert(reportingCsv(p).includes(p.deliverySha256));
 ...
 const old=runtimeReleaseHistory.find(r=>r.version==='3.14.0');
...
test('private selection stores a complete 3.14.1 package with literal labels and preserves existing defaults',async()=>{
 const f=await cacheSelectionFixture();try {
  ...
  const runtime=state.runtimes.find(r=>r.version==='3.14.1').pin;
  ...
  const saved=await f.api('/test-api/runtime-selection');assert.equal(saved.selected.runtime.runtimeVersion,'3.14.1');
  ...
  assert.equal((await describeCandidate('test-site',stored)).descriptor.runtime.runtimeVersion,'3.14.1');
  for(const name of ['ads.js','ads.min.js']){
   const source=new TextDecoder().decode(stored.files[name]);assert(source.includes('Variant'));assert(!source.includes('tessera_ab'));assert(!source.includes('Varijant'));
  }
 }finally{f.close();}
});
