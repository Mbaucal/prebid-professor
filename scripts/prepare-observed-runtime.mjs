...
export async function observedRuntimeSource(root){
...
export async function prepareObservedRuntime(root){
  const {codeSha256,components}=await observedRuntimeSource(root);
  const release=runtimeReleaseHistory.find(r=>r.id==='tessera-observed-preview-1');
  if(!release)throw Error('Register the observed runtime before building.');
  ...
  await writeFile(resolve(root,'.generated/runtime-observed-manifest.mjs'),`// Generated from verified versioned sources.\nexport const descriptor=${JSON.stringify(descriptor)};\nexport const sourceComponents=${JSON.stringify(components)};\n`);
}
