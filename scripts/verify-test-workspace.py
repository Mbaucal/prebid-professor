...
def exercise(page):
    ...
    try:
        ...
        expect(page.locator('#release-history article[data-available="true"]')).to_have_count(3)
    ...
    page.goto(origin+'/runtime-selection')
    expect(page.locator('#runtime')).to_be_enabled()
    page.locator('#runtime').select_option(label='3.11.0-tessera.preview.1 · preview')
    page.locator('#allow-preview').check()
    page.locator('#save-selection').click()
    expect(page.locator('#message')).to_contain_text('Runtime selection saved')
    page.get_by_role('link',name='Back to Generate').click()
    page.locator('#generate').click()
    expect(page.locator('#source')).to_contain_text('__tesseraRuntimeDiagnostics',timeout=90000)
    page.locator('#ack').check()
    page.locator('#save').click()
    expect(page.locator('#releases article')).to_have_count(2,timeout=90000)
    check('New diagnostic version is explicitly selected and saved without replacing the old package',page.evaluate('typeof window.__tesseraRuntimeDiagnostics')=='undefined')
