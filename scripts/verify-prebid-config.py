...
def exercise(page):
    ...
    catalog=origin+'/test-api/prebid/versions'
    public_catalog='https://js-download.prebid.org/versions'
    fixture_headers={'content-type':'application/json','x-tessera-local-fixture':'sqlite-fake-r2','access-control-allow-origin':'*'}
    def catalog_failure(route):
        route.fulfill(status=503,headers=fixture_headers,body=json.dumps({'error':'Catalog temporarily unavailable'}))
    page.route(catalog,catalog_failure)
    page.route(public_catalog,catalog_failure)
    ...
    expect(page.locator('#version-note')).to_contain_text('Version list unavailable')
    before=page.evaluate('planRequest()')
    page.locator('#version-options summary').click()
    page.locator('#load-versions').click()
    expect(page.locator('#version-note')).to_contain_text('Version list unavailable')
    for selector in ['#download-config','#open-builder','#save-prebid','#save-plan']:
        expect(page.locator(selector)).to_be_enabled()
    check('Catalog 503 leaves all unrelated actions and unsaved fields intact',page.evaluate('planRequest()')==before and 'error' not in page.locator('#message').get_attribute('class'))
    page.unroute(public_catalog,catalog_failure)
    pending=[]
    page.route(public_catalog,lambda route:pending.append(route))
    page.locator('#load-versions').click()
    expect(page.locator('#load-versions')).to_be_disabled()
    page.locator('#plan-version').fill('11.33.0')
    expect(page.locator('#download-config')).to_be_enabled()
    expect(page.locator('#bidders [data-field="params"]')).to_be_enabled()
    # The fallback request is fulfilled locally, never sent to Prebid.
    page.wait_for_timeout(100)
    assert len(pending)==1
    request=pending[0].request
    check('Browser fallback is a fixed public GET without credentials, referrer or partner payload',request.method=='GET' and request.post_data is None and not request.headers.get('cookie') and not request.headers.get('referer') and request.url==public_catalog)
    pending[0].fulfill(status=200,headers=fixture_headers,body=json.dumps({'versions':['11.34.0','11.33.0']}))
    expect(page.locator('#version-note')).to_contain_text('Version list ready')
    expect(page.locator('#plan-version')).to_have_value('11.33.0')
    check('Slow catalog request does not freeze editing or replace a version entered while waiting',True)
    page.unroute(public_catalog)
    page.route(public_catalog,lambda route:route.fulfill(status=200,headers=fixture_headers,body=json.dumps({'versions':['11.34.0','11.33.0']})))
    page.locator('#available-versions').select_option('11.34.0')
    expect(page.locator('#plan-version')).to_have_value('11.34.0')
    page.locator('#version-options summary').click()
    ...
    second.locator('#save-later summary').click()
    ...
    # A catalog outage and an uncertain write are different states.
    # Successful catalog recovery must not clear a genuine write lock.
    before=page.evaluate('planRequest()')
    page.route(origin+'/test-api/prebid-settings',lambda route:route.fulfill(status=503,headers=fixture_headers,body=json.dumps({'error':'Save result could not be confirmed'})))
    page.locator('#save-prebid').click()
    expect(page.locator('#message')).to_contain_text('Save result could not be confirmed')
    expect(page.locator('#download-config')).to_be_disabled()
    page.locator('#version-options').evaluate('(element)=>element.open=true')
    page.locator('#load-versions').click()
    expect(page.locator('#version-note')).to_contain_text('Version list ready')
    expect(page.locator('#save-prebid')).to_be_disabled()
    expect(page.locator('#download-config')).to_be_disabled()
    check('Catalog success never unlocks an uncertain save or resets the edited form',page.evaluate('planRequest()')==before)
