...
export async function buildConfigurableABPackage({base, previousArchive, settings: input}) {
  ...
  const templates = {};
  for (const variant of ['A', 'B']) templates[variant] = await compact(configuredCacheArm(Buffer.from(base).toString(), variant, settings, SENTINEL));
  const identity = sha256(Buffer.concat([
    ...
    templates.A, templates.B, Buffer.from(configuredCacheLoader.toString()),
    // Include inherited loader implementation and compiler/minifier output.
    await compact(configuredCacheLoader({release: SENTINEL, settings, arms: {A: {}, B: {}}, positions: []})),
  ]));
  ...
  files['ads.js'] = await compact(configuredCacheLoader(config));
