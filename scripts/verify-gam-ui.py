...
try:
    ...
    with sync_playwright() as p:
        ...
        page.get_by_label('Tessera sajt',exact=True).select_option('test-site')
        ...
        page.locator('.gam-result').get_by_text('✓ Upisano u sajt: example.invalid',exact=True).wait_for()
        expect(page.locator('.gam-result .gam-site-sync')).to_contain_text('20 novih ad unita · 7 novih mapa')
        page.locator('.gam-result .gam-site-sync').scroll_into_view_if_needed()
        page.screenshot(path=str(out/'gam-site-synced.png'),full_page=False)
