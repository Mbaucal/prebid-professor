...
export const sourceFiles=[
  ...
  'worker/runtime-variant/targeting.mjs','worker/runtime-variant/compiler.mjs','worker/runtime-variant/artifact-candidate.mjs'
];
const hash=bytes=>createHash('sha256').update(bytes).digest('hex');
export async function variantRuntimeSource(root){
...
export async function prepareVariantRuntime(root){
  const {codeSha256,components}=await variantRuntimeSource(root);
  const release=runtimeReleaseHistory.find(r=>r.id==='variant-labels-preview-1');
  ...
  await writeFile(resolve(root,'.generated/runtime-variant-manifest.mjs'),`// Generated from verified versioned sources.\nexport const descriptor=${JSON.stringify(descriptor)};\nexport const sourceComponents=${JSON.stringify(components)};\n`);
}
