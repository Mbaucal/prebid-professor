/** Real compiled TEST or production Worker and native workerd fetch; all Google responses stay local.
...
const config = JSON.parse(await readFile(production ? 'wrangler.jsonc' : 'ops/runtime-test/wrangler.active.jsonc', 'utf8'));
assert.equal(config.name, production ? 'prebid-professor' : 'prebid-professor-test');
const origin = production ? 'https://prebid-professor.mbaucal.workers.dev' : config.vars.TEST_PUBLIC_ORIGIN;
const apiPath = production ? '/api/integrations/gam' : '/test-api/integrations/gam';
...
const authPrefix = production ? '' : 'TEST_';
const bindings = {...config.vars, [authPrefix+'ADMIN_EMAIL']:'fixture@example.invalid',
  [authPrefix+'ADMIN_PASSWORD']:randomBytes(24).toString('base64url'),
  [authPrefix+'SESSION_SECRET']:randomBytes(48).toString('base64url'),
...
async function call(path, body) {
  const response = await mf.dispatchFetch(origin + apiPath + path, {
...
try {
  ...
  checked('GAM status requires the application login', (await call('/status')).status === 401);
  const login = await mf.dispatchFetch(origin + '/api/auth/login', {method:'POST', redirect:'manual',
    headers:{origin, 'content-type':'application/x-www-form-urlencoded'},
    body:new URLSearchParams({email:bindings[authPrefix+'ADMIN_EMAIL'], password:bindings[authPrefix+'ADMIN_PASSWORD']}).toString()});
  assert.equal(login.status, 303);
  cookie = login.headers.get('set-cookie').split(';')[0];
  const status = await call('/status');
  checked('Authenticated status reports a configured empty connection store', status.status === 200 && status.data.configured && status.data.connections.length === 0);
  const crossOrigin = await mf.dispatchFetch(origin + apiPath + '/connect', {method:'POST',
    headers:{cookie, origin:'https://unexpected.invalid', 'content-type':'application/json'}, body:'{}'});
  checked('Cross-origin connection is rejected before Google or storage access', crossOrigin.status === 403 && events.length === 0);
  ...
  const report = {scope:`Local compiled ${production ? 'production' : 'TEST'} Worker in workerd; synthetic Google only`, checks, events,
    realGoogleRequests:0, realInventoryWrites:0, passed:true};
  await mkdir('.generated/gam-ui-evidence', {recursive:true});
  await writeFile(`.generated/gam-ui-evidence/workerd${production ? '-production' : ''}.json`, JSON.stringify(report, null, 2));
