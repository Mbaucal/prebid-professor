...
export const sourceFiles = [
  ...
  'worker/runtime-demand-v1/snapshot.mjs','worker/runtime-demand-v1/requirements.mjs',
  'worker/runtime-demand-v1/browser-demand.mjs','worker/runtime-demand-v1/compiler.mjs',
  'worker/runtime-demand-v1/contract.mjs','worker/runtime-demand-v1/artifact-candidate.mjs',
  'scripts/prepare-demand-runtime.mjs'
];
const hash = bytes => createHash('sha256').update(bytes).digest('hex');
export async function demandRuntimeSource(root) {
...
export async function prepareDemandRuntime(root) {
  const {codeSha256,components} = await demandRuntimeSource(root);
  const release = runtimeReleaseHistory.find(r=>r.id==='tessera-demand-v1');
  if (!release) throw Error('Register the demand runtime before building.');
  assertRuntimeReleaseSource(codeSha256,release);
  const {id,version,configSchemaVersion,channel,capabilities}=release;
  const browserSources={};
  for(const path of ['worker/runtime-demand-v1/browser-demand.mjs']){
  ...
  await writeFile(resolve(root,'.generated/runtime-demand-browser-source.mjs'),
  ...
  await writeFile(resolve(root,'.generated/runtime-demand-manifest.mjs'),
