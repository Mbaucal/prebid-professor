...
import {fixture} from '../tests/gam/fixture.mjs';
const f=fixture();
...
await gamResponse(new Request(address+base+'/connect',{method:'POST',headers:{origin:address,'content-type':'application/json'},body:JSON.stringify({networkCode:'123456',credentials:account})}),f.env,'fixture@example.invalid',{base,clientFactory:()=>f.client});
createServer(async(req,res)=>{
 try{
  ...
  else if(url.pathname.startsWith(base+'/')){let body='';for await(const chunk of req)body+=chunk;response=await gamResponse(new Request(url,{method:req.method,headers:req.headers,body:['GET','HEAD'].includes(req.method)?undefined:body}),f.env,'fixture@example.invalid',{base,clientFactory:()=>f.client});}
