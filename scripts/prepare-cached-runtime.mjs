...
export async function cachedRuntimeSource(root){
...
export async function prepareCachedRuntime(root){
  const {codeSha256,components}=await cachedRuntimeSource(root);
  const release=runtimeReleaseHistory.find(r=>r.id==='tessera-cache-preview-1');
  if(!release)throw Error('Register the cached runtime before building.');
