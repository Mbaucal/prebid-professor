...
def exercise(page):
    ...
    try:
        ...
        page.get_by_label('Allowed sizes',exact=True).last.fill('320x250')
        ...
        page.get_by_label('Desktop auto-close (seconds)',exact=True).fill('0')
    ...
    expect(page.get_by_label('Position ID',exact=True).last).to_have_value('Overlay')
    expect(page.get_by_label('Display',exact=True).last).to_have_value('takeover')
    expect(page.get_by_label('Desktop auto-close (seconds)',exact=True)).to_have_value('0')
    expect(page.get_by_label('Size map',exact=True).last).to_have_value('overlay_map')
    check('Edited configuration and TakeOver controls survive browser reload',True)
    page.locator('#confirm-draft').check()
    page.locator('#save-site').click()
    ...
    check('Generate uses the saved TakeOver choice without a second checkbox',page.locator('#takeover').count()==0)
