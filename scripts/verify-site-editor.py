...
def exercise(page):
    ...
    try:
        ...
        page.get_by_label('Mobile / tablet width (px)',exact=True).fill('320')
    ...
    expect(page.locator('#takeover-enabled')).to_be_checked()
    expect(page.get_by_label('Desktop auto-close (seconds)',exact=True)).to_have_value('0')
    expect(page.get_by_label('Mobile / tablet width (px)',exact=True)).to_have_value('320')
