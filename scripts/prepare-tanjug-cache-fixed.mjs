...
import {CACHE_RELEASE,cacheArm,fullCacheLoader} from '../worker/experiments/full-cache-v2.mjs';
...
const out=new URL('../.generated/tanjug-cache-fixed/',import.meta.url);
...
put('UPUTSTVO.txt',`TANJUG — ${CACHE_RELEASE}\n\nObjavi CEO ZIP u isti Pages projekat. HTML i GAM ključ Variant ostaju isti.\nA: postojeća sveža aukcija. B: nova aukcija uz validne neiskorišćene keširane bidove.\nKeš najviše 60 s, uz kraći originalni TTL i provere konteksta. Svih 19 pozicija.\nRefresh intervali nisu promenjeni. Ovo nije cache-first niti brži refresh.\n\nAdVariant.inspect() pokazuje A/B i skriptu. U B, AdBidCache.inspect() pokazuje\nbroj svežih/keširanih izbora i razloge odbacivanja. Keš ne garantuje prikaz: GAM\ni dalje odlučuje. Ako CMP kontekst nije poznat, reuse se ne uključuje.\n\nZabeleži vreme objave i u GAM-u poredi samo novi period, da ne pomešamo prethodni\nA/A test sa ovim A/B testom. Za povratak koristi prethodni Pages deployment.\n`);
