...
export async function nextRuntimeSource(root){
...
export async function prepareNextRuntime(root){
  const {codeSha256,components}=await nextRuntimeSource(root);
  const release=runtimeReleaseHistory.find(r=>r.id==='tessera-positions-preview-1');
  if(!release)throw Error('Register the ad-position runtime before building.');
  ...
  await writeFile(resolve(root,'.generated/runtime-next-manifest.mjs'),`// Generated from verified versioned sources.\nexport const descriptor=${JSON.stringify(descriptor)};\nexport const sourceComponents=${JSON.stringify(components)};\n`);
}
